...
import { Play, Download, Loader2, CheckCircle2, Plus, Trash2, Search, Zap } from "lucide-react";
...
export default function SignalIntelPage() {
  ...
  const run = useCallback(async () => {
    ...
    // Pre-expand all target companies
    const initExpanded = {};
    ...
    try {
      const res = await fetch(`${API_URL}/api/signal-intel`, {
        ...
        signal: ctrl.signal,
        body: JSON.stringify({
          target_companies: validCompanies.map(c => ({
            name: c.name.trim(),
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (evt.type === "heartbeat") {
              ...
              setTimeout(() => setNewRowIds(prev => {
                const n = new Set(prev); n.delete(rowId); return n;
              }), 1200);

            } else if (evt.type === "signals_ranked") {
              // Replace rows for this company with ranked versions
              setAllSignals(prev => {
  ...
  const filtered = allSignals.filter(r => {
    ...
    if (search) {
      ...
      const hay = `${r.signal_title} ${r.summary} ${r.company} ${r.signal_type}`.toLowerCase();
  ...
  // Sort companies: ranked first (has importance != "—"), then by count desc
  const sortedCompanies = Object.keys(grouped).sort((a, b) => {
    const aHasRank = grouped[a].some(r => r.importance !== "—");
    const bHasRank = grouped[b].some(r => r.importance !== "—");
    if (aHasRank !== bHasRank) return aHasRank ? -1 : 1;
    return grouped[b].length - grouped[a].length;
  });

  // Sort signals within each company by importance
  ...
  return (
    <div className={s.page}>
      ...
      <main className={s.main}>
        {/* Your company (optional context) */}
        <div className={s.card}>
          <div className={s.cardTitle}>
            <span>Your Company Context</span>
            <span style={{ fontSize: 11, fontWeight: 400, color: "#475569" }}>optional — used to rank signals by relevance to your sales motion</span>
          </div>
          <div className={s.configGrid}>
            ...
            <div className={s.fieldGroup}>
              <label className={s.fieldLabel}>Key Triggers to Watch <span className={s.optional}>(optional)</span></label>
        ...
        {allSignals.length > 0 && (
          <div className={s.resultsArea}>
            ...
            {/* Category + importance + search filters */}
                    ...
                    <div key={compName} className={s.compGroup}>
                      <button className={s.compGroupHeader} onClick={() => toggleComp(compName)}>
                        <div style={{ display: "flex", flexDirection: "column", gap: 2, flex: 1 }}>
                          ...
                          {rows[0]?.domain && (
                            <span className={s.compGroupDomain}>{rows[0].domain}</span>
                          )}
                        </div>
